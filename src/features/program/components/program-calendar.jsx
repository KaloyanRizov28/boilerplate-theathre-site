...
const getMonthCalendarDetails = (year, month, weekStartsOn = 1) => {
  ...
  if (isNaN(localDateForMonthName.getTime()) || isNaN(internalDate.getTime())) {
  ...
  const monthNameForDisplay = localDateForMonthName.toLocaleString('bg-BG', { month: 'long' })
...
const ProgramCalendar = ({
  ...
  const secondMonthInWindow = useMemo(() => {
    ...
    const details = getMonthCalendarDetails(year, month, 1)
    return { month, year, name: details.monthName }
  }, [calendarNavBaseMonth, calendarNavBaseYear])
